...
\begin{itemize}[leftmargin=1.2em]
  ...
  \item Key findings:
    \begin{itemize}[leftmargin=1.2em]
      ...
      \item Consistent observation of vertical CTI across all CCD modules when using LBC operating parameters
      \item 
    \end{itemize}
...
The CCD test setup is shown in Fig.~\ref{fig:JHUChamberExterior}. The setup was part of two test systems designed and provided by Johns Hopkins University, constructed in an ISO-6 cleanroom at the University of Chicago, shipped to UW for surface testing, and subsequently sent to LSM for underground testing. Each system was designed to accommodate up to four CCD dice or modules; however, only three were tested simultaneously due to electronics availability. The systems consisted of the following components:
...
\item Cryostat and vacuum chain: The test system was based on a stainless-steel vacuum cryostat mounted on top of a wheeled cart for transport. Below the chamber was a Pfeiffer HiCube 80 Eco turbomolecular pump, electrically isolated from the test chamber via a ceramic vacuum fitting. The residual pressure inside the cryostat was monitored by a Pfeiffer PTR91 dual Pirani/cold cathode\footnote{Since the cold cathode operates via high-voltage glow discharge, which can generate detectable photons and electromagnetic interference, the gauge was powered off during CCD testing.} gauge and reached ${\lesssim}10^{-4}\text{\,mbar}$ at room temperature. The pump and pressure gauge had external Ethernet connections for monitoring (Pfeiffer CenterOne) and control (Perle Iolan DS1).
    
\item Thermal chain: The cryostat was equipped with a Cryotel GT cryocooler with active vibration cancellation. The air-side cooling fins were enclosed by a 3D-printed shroud with a fan to prevent overheating. Inside the cryostat, the aluminum CCD boxes were thermally coupled to the cryocooler cold end via a copper bar. A layer of Kapton tape was used to electrically isolate the bar from the cryocooler, and a plastic KF fitting was used to isolate the cryocooler body from the cryostat. Resistance temperature detectors (RTDs) were positioned on the cryocooler cold end, on the thermal bar, and on the outer CCD boxes (two RTDs). The RTDs and a resistive heater mounted to the copper bar were connected in a PID control loop with a Lakeshore 336 temperature controller via a custom vacuum-air interface board with D-sub connections. A PTFE table provided mechanical support to the CCD boxes, as well as thermal and electrical isolation from the chamber walls.

An important difference between the thermal chain used during surface testing and that used during underground testing concerned the active balancer controlling the cryocooler. Following surface testing, the cryocoolers were sent to Ametek for inspection and repair, as multiple issues were reported, including overcurrent incidents that caused instrument shutdown, audible ringing from the active balancer vibrating against the cryocooler, and low-frequency noise introduced by the active vibration control system. Despite intervention by Ametek technicians upon arrival and installation at LSM, the cryocooler configuration—consisting of an AVC balancer and a second-generation AVC controller—continued to exhibit overcurrent and low-frequency noise. 

The overcurrent issue that prevented prolonged cryocooler operation was resolved by using a second-generation AVC balancer from Ametek. However, this AVC model consistently introduced low-frequency noise, resulting in overall readout noise levels higher than those previously observed. As a result, a first-generation AVC balancer was used throughout the testing, as it provided stable, continuous operation and acceptable noise levels. Figure~\ref{fig:psd_avc} shows the noise level evolution across three configurations:
...
\item Slow control: This system monitored and controlled the cryostat service equipment (cryocoolers, vacuum pumps, temperature controllers) using custom C-based back-end software with a PHP-based front-end interface \cite{DAMIC-M:2024ooa,slowcontrol}. Communication with the electronics modules was performed over a local wired network, converting from serial protocols to TCP when necessary. The system status was logged in a MySQL database, and alarms were triggered if parameters exceeded predefined bounds. 

\item Data acquisition: This system was responsible for CCD control and data taking. A Wiener VME195x crate housed the test chamber, which contained three Acquisition and Control Modules (ACMs), each connected to a front-end board on the cryostat. The ACMs generated bias and clock voltages for the CCDs and processed their output signals through four channels, one per CCD, during module testing \cite{Gaior:2023wfr}. A dedicated clock-synchronization board enabled synchronous operation of the ACMs during CCD readout. A suite of Acopian linear power supplies provided low-noise DC voltages to the VME crates (±15 V, +30 V). An additional Linux server set the CCD operating parameters, executed readout, and served as local data storage.
...
Control and communication with each ACM board were performed using the local DAQ (LDAQ), comprising ACMDAQ, \texttt{acmpy}, a sequencer, and a decoder \todonote{cite the ACMDAQ/acmpy GitHub repository and the SSM (Sravan?) thesis describing the LDAQ software stack.}. ACMDAQ is a C++ server that communicates directly with the ACM via hexadecimal messages over Ethernet. The Python package \texttt{acmpy} interfaces with ACMDAQ and provides high-level control of the CCD. The sequencer file, written in the LSST sequencer language \todonote{cite the LSST camera sequencer language reference, or the DAMIC-M internal note describing its adaptation.}, is compiled within \texttt{acmpy} and transmitted to the ACM FPGA to define the clock sequencing. Each ACM board is operated by an independent LDAQ instance, which can run either standalone or under the central DAQ (CDAQ) \todonote{cite the CDAQ software/documentation.}.

For each tested module, a dedicated sequencer and configuration file were prepared to define timing and voltage parameters. A summary of the timing parameters used for production modules is presented in Table.~\ref{tab:sequencer_timing_dm13}. The table shows an example corresponding to a full-array image acquired with no binning, no image cuts, and single-skip (NDCM=1) readout.
%
\begin{table}[h!]
\centering
...
\end{table}
%
In addition to the sequencer file, a configuration file in \texttt{.bcf} format specified the bias and clock-rail voltages applied during operation. The configuration parameters corresponding to the example above are summarized in Table.~\ref{tab:config_dm13}.
...
The amplifier wire-bonding configuration and corresponding ACM channel mapping are shown in Table.~\ref{tab:amp_mapping}. The charge-transfer direction, defined through sequencer and configuration function pointers, depended on the bonding configuration to ensure that charge was transferred toward the active output amplifiers. For the configuration shown here, the vertical transfer pointer was set to \texttt{Vtransfer21}.
...
A total of seven image types were acquired at high (185 K) and low (135 K) temperatures to characterize detector performance under different operating conditions. The datasets include full-array exposures, serial-register readouts, and multi-skip images. A summary of the image configurations and their primary objectives is presented in Table~\ref{tab:image_summary}.
...
In contrast to on-surface testing, the high-temperature measurements in this campaign were performed after low-temperature operation, during the controlled warm-up from 135 K toward room temperature. This sequence was chosen to mimic the thermal cycle implemented in the LBC, in which erase and epurge steps are performed at 185 K and 135 K (with only epurge at 135 K). Unlike standard LBC operations, no erase or epurge steps were performed at 150 K during these tests.


\subsection{Acquisition sequence} 

The following image acquisition sequence was performed at 135 K and 185 K. The number of exposures for each image type and the corresponding readout times are summarized below.
...
The total acquisition time at 185 K was 5\,hr 22\,min 8\,s. This sequence of data acquisitions allowed for two module batches to be tested every week.
...
As in the die- and module-level surface testing, a series of measurements was performed at 185~K to facilitate the efficient identification of defects in each CCD within each module. Although 185~K is not the nominal operating temperature of the DAMIC-M experiment, additional images were acquired at this temperature to further characterize the overall Skipper-CCD performance of each module. These measurements were used to evaluate key performance metrics, including readout noise, single-electron resolution, dark current, and gain, and to investigate potential temperature-dependent effects.
...
Across all modules and CCDs, the serial-register defect count remains low, with a global mean of 0.24 defects per CCD. The majority of CCDs exhibit no detectable defects, while a small subset shows a single defective column in the serial register. No CCD was observed to contain more than one serial-register defect. 
%
A limited number of modules contain serial-register defects in more than one CCD extension. In particular, modules DM-05, DM-06, DM-13, DM-18, DM-20, and DM-23 each show defects in at least two CCDs. Nevertheless, even in these cases, the multiplicity of defects per CCD remains limited to a single column, and the overall incidence of defects remains low across the production set.
...
When the test system is operated at 135\,K, the dark current decreases with temperature. To estimate skipper performance under these conditions, 10 pixels from the serial register were transferred into the floating gate before measurement to ensure sufficient charge in the image pixels. This was done by collecting six \texttt{Image\_3\_135}.
...
\end{figure}
The skipper performance metrics extracted from the Poisson–Gaussian fits to the \texttt{Image\_3\_135} data are summarized in Figs.~\ref{fig:Single_e_res_all}--\ref{fig:DC_all}. These figures present the fitted single-electron resolution ($\sigma$), amplifier gain, and dark current for all four CCD extensions (A–D) across modules DM-01 to DM-28. The dotted horizontal line indicates the mean value across all extensions, and the shaded band represents $\pm 1\sigma$ about the mean.

Figure~\ref{fig:Single_e_res_all} shows the single-electron resolution derived from the Gaussian width of the fitted charge peaks. 


Figure~\ref{fig:Gain_all} presents the amplifier gain obtained from the spacing between consecutive peaks in the charge distribution. 

Figure~\ref{fig:DC_all} shows the dark current inferred from the fitted Poisson rate parameter $\lambda$, converted to units of $e^-$/pixel/day.
...
\begin{figure}[h!]
    ...
    \caption[Ratio of 135\,K to 185\,K dark current, all modules]{Ratio of the 135\,K to 185\,K dark current (Fig.~\ref{fig:DC_all} over Fig.~\ref{fig:DC_high_all}) for each CCD extension (A--D) across modules DM-01 to DM-28, on a logarithmic scale. The ensemble mean ratio is $3.0\times10^{-3}$. \todonote{Note the y-axis is capped at $10^{-2}$ in the source plot; several modules (e.g.\ DM-08, DM-10, DM-12, DM-24--DM-26) have ratios that exceed this range and are clipped -- consider requesting an unclipped version for the final report.}}
...
X-rays from the radioactive decay of $^{55}$Fe in the iron foils attached to the storage box lids (module packaging described in \cite{Traina2025}) produce ionization events throughout the CCD bulk, including both front- and backside interactions.
These monoenergetic X-rays provide a controlled probe of the detector response and are used to characterize the keV-scale energy calibration, charge transfer inefficiency (CTI), and lateral charge diffusion in the sensors~\cite{Janesick:2001}.

Images are processed by identifying contiguous pixels above background noise and grouping them into reconstructed ``clusters.'' 
...
Two clearly separated populations are observed, corresponding to front- and backside interactions. 
...
In particular, Image\_5 was selected: a cropped region of the CCD centered on the area illuminated by the iron foils and read out with 500 skips. 
This configuration enhances the signal-to-noise ratio and enables high-resolution measurements of the energy response and CTI-sensitive cluster morphology in the region of interest.
...
An example composite cluster illustrating these regions is shown in Fig.~\ref{fig:front_composite_example}.
...
Systematic differences between opposite regions indicate preferential redistribution of charge along one direction, consistent with asymmetric transfer effects.
...
\begin{figure}[h!]
    ...
    \caption[Parallel CTI-sensitive fraction, all modules]{Parallel CTI-sensitive fraction $f_{\mathrm{Above}}$ from front-side $^{55}$Fe clusters (500-skip Image\_5 data), for each CCD extension (A--D) across modules DM-01 to DM-28. The ensemble mean is $5\times10^{-3}$; DM-16 and DM-19 stand out as having the largest parallel CTI-sensitive fractions.}
...
\begin{figure}[h!]
    ...
    \caption[Serial CTI-sensitive fraction, all modules]{Serial CTI-sensitive fraction $f_{\mathrm{Right}}$ from front-side $^{55}$Fe clusters (500-skip Image\_5 data), for each CCD extension (A--D) across modules DM-01 to DM-28. The ensemble mean is $3\times10^{-3}$, generally smaller than the parallel fraction shown in Fig.~\ref{fig:cti_front_above_all}.}
...
For the back-side composite cluster, one-dimensional projections are constructed within fixed edge slabs of the composite grid in order to isolate the extended tails of the charge distribution.
...
The projection integrals probe the amount of charge residing in extended tails, while the RMS and skewness quantify the width and directional asymmetry of the charge distribution. 
...
The RMS of the projections provides a measure of the width of the average back-side cluster distribution in each direction. 
...
\begin{figure}[h!]
    ...
    \caption[RMS of serial projection, back-side events, all modules]{RMS of the serial-direction ($\Delta x$) projection of the back-side composite $^{55}$Fe cluster, for each CCD extension (A--D) across modules DM-01 to DM-28. The ensemble mean is $1.470$\,pixels.}
...
\begin{figure}[h!]
    ...
    \caption[RMS of parallel projection, back-side events, all modules]{RMS of the parallel-direction ($\Delta y$) projection of the back-side composite $^{55}$Fe cluster, for each CCD extension (A--D) across modules DM-01 to DM-28. The ensemble mean is $1.500$\,pixels, closely matching the serial-direction RMS of Fig.~\ref{fig:cti_back_serial_rms_all} and consistent with isotropic diffusion dominating the back-side cluster shape across the ensemble.}
...
\begin{figure}[h!]
    ...
    \caption[Cluster energy vs.\ $\sigma_{xy}$, VR comparison]{Reconstructed cluster energy versus lateral spread $\sigma_{xy}$ for DM01, Ext.\ 4, at the nominal LBC operating point, $\mathrm{VR}=-3$\,V (left), and at $\mathrm{VR}=-6$\,V (right). The low-$\sigma_{xy}$ (front-side) and high-$\sigma_{xy}$ (back-side) populations are both visible, each showing the K$\alpha$/K$\beta$ two-line structure. Lowering VR shifts the front-side population to higher, more nearly nominal energies and increases the population density, consistent with the reduced energy-scale shift discussed in the text.}
    \label{fig:energy_vs_sigmaxy}
\end{figure}

\begin{itemize}
\item \textbf{Line reconstruction.} For each CCD extension, the front-side K$\alpha$ and K$\beta$ peaks are fit in the reconstructed-energy spectrum. Figure~\ref{fig:kev_lines_bymodule} shows the fitted K$\alpha$ and K$\beta$ line energies for every module and CCD in the underground campaign.
\item \textbf{Systematic energy-scale shift.} Across the ensemble, the reconstructed line energies are systematically lower than the nominal values: K$\alpha \approx 5.761$\,keV (expected 5.90\,keV) and K$\beta \approx 6.276$\,keV (expected 6.49\,keV), a shift of roughly 2--3\%, observed consistently across CCDs and modules.
\item \textbf{Pixel-charge nonlinearity.} A dedicated high-statistics dataset near $500\,e^-$ was used to study the response of the skipper amplifier as a function of reconstructed charge. Figure~\ref{fig:nonlinearity} shows a clear nonlinear deviation between the reconstructed and expected pixel charge that grows with charge. Applying a nonlinear correction of this form is sufficient to account for the observed K$\alpha$/K$\beta$ energy-scale shift; however, the magnitude of the effect depends on both the amplifier and the operating parameters used, and is examined in more detail in a complementary, forthcoming study of amplifier nonlinearity. \todonote{Cross-reference the forthcoming amplifier-nonlinearity study once it has a docdb number or draft to cite.}
\item \textbf{Bias-voltage dependence.} Parameter scans were performed over the module biases and most rail voltages to identify the origin of the shift. The largest effect was found when lowering the reference voltage ($\mathrm{VR}$) from its nominal LBC science-run value of $-3$\,V to $-6$\,V, which reduces the observed nonlinear effect: Fig.~\ref{fig:vr_scan} compares the front/back energy spectra at the two settings, showing that $\mathrm{VR}=-6$\,V brings the front- and back-side K$\alpha$ peaks into much closer agreement than the nominal $\mathrm{VR}=-3$\,V setting. The revised parameters were not observed to affect gain, single-electron resolution, or dark current relative to the nominal LBC operating point.
\end{itemize}
...
\begin{figure}[h!]
    ...
    \caption[K$\alpha$/K$\beta$ line energies by module]{Fitted front-side Mn K$\alpha$ (top, nominal 5.89\,keV) and Mn K$\beta$ (bottom, nominal 6.49\,keV) line energies for each CCD extension across all production modules tested underground, from 500-skip Image\_5 data at 135\,K. Both lines are reconstructed systematically below their nominal values (ensemble means 5.761\,keV and 6.276\,keV respectively), motivating the nonlinearity and bias-voltage studies below.}
...
\begin{figure}[h!]
    ...
    \caption[Bias-voltage dependence of the energy-scale shift]{Front- ($\sigma_{xy}<1$, red) and back-side ($\sigma_{xy}>1$, blue) energy spectra for a representative extension (DM01, Ext.\ 4) at the nominal LBC operating point, $\mathrm{VR}=-3$\,V (left), and at $\mathrm{VR}=-6$\,V (right). Lowering VR shifts both lines closer to their nominal energies and brings the front- and back-side K$\alpha$ peaks into closer agreement ($\mu_{\mathrm{front}}=5.834$\,keV vs.\ $\mu_{\mathrm{back}}=5.873$\,keV at $\mathrm{VR}=-6$\,V, compared to $\mu_{\mathrm{front}}=5.728$\,keV vs.\ $\mu_{\mathrm{back}}=5.900$\,keV at $\mathrm{VR}=-3$\,V). \todonote{Non-linearity at VR=-6\,V still needs to be characterized with higher statistics; note this as a follow-up in the discussion.}}
    \label{fig:vr_scan}
\end{figure}

\subsection{Crosstalk}

Crosstalk occurs when a pixel in a source CCD containing a genuine charge signal induces a small signal in the corresponding pixel of another CCD, referred to as the destination CCD. This effect originates from electronic coupling between the readout channels. When accumulated over many events, it leads to a systematic linear correlation between the charges measured in corresponding pixels of the source (src) and destination (dst) CCDs. 
...
Figure \ref{fig:CrosstalkDM16} shows the pairwise crosstalk characterization between the four CCD channels. Each panel corresponds to an ordered pair (src, dst), where the pixel charge of the source CCD is indicated on the horizontal axis and the pixel charge of the destination CCD on the vertical axis. Each blue point represents the charge for the selected seed pixels for both source and destination CCDs. The black line represents the computed linear correlation between the source and destination pixel charges, where its slope corresponds to the crosstalk coefficient, and the red dashed line represents the linear correlation corrected by the null hypothesis.
...
From this study, it was concluded that a single crosstalk correction matrix can be consistently applied across all ACMs and modules. It should be noted, however, that not all modules were analyzed, as the current methodology is not suitable for determining crosstalk in modules exhibiting high dark current.
...
The full per-module breakdown of these metrics (operational CCDs, gain, SER, dark current, and CTI fractions) is given in Table~\ref{tab:summary_metrics}.
