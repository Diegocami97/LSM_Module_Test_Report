...
At 135\,K, the test system's dark current is greatly reduced relative to higher-temperature operation. To estimate skipper performance under these conditions, we transferred 10 pixels from the serial register into the floating gate before measurement to ensure sufficient charge in the image pixels. This was done by collecting six \texttt{Image\_3\_135}.
...
Here, $ \ alpha _ {\ mathrm {null}} $: the horizontal axis shows the source CCD pixel charge, and the vertical axis shows the destination CCD pixel charge. It is computed by randomizing the spatial positions of all seed pixels in the source image, breaking the correlation.
